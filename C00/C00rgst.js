...
function sendFilePrc(updflg){     //新增資料及修改程序       
    ...
    if (tbno==0){
	    var c00elements=document.getElementsByName('c00update');	
    }else{
	     var c00elements=document.getElementsByName('c0Zupdate');	
	}
	for(var q=1;q<c00elements.length;q++){  	    //開始堆疊待異動資料陣列
		 tbjsn.push(c00elements[q].value);	   
	}

	for(var j=1;j<c00elements.length;j++){
        if(c00elements[j].value.trim()=="" ){				      
		    filtermsg(c00elements[j],"不得空白");			 
		    return false ;
        }else if(c00elements[j].value == 0){		  		    		      
			  filtermsg(c00elements[j],"不得為 0");
		      return false ;		 
		}else if(j==1 && tbno==1 && right(c00elements[j].value,1)!=1){ 
		      filtermsg(c00elements[j],"非該旬首日");
		      return false ;
		}else{
		   if(c00elements[j].nextSibling ){		              
...
function modifyFields(tbno,txtword,ajTable){   //新增修改時出現之欄位
    if(tbno==0){   //如果異動表頭資料
	    ...
	    oTd.colspan=3;
	    oTd.innerHTML="<input type='number' name='c00update' id='crncyabbrv' class='txt' value=0 style='width:25%;'  />";                             	 
	    var oTr=ajTable.insertRow(ajTable,ajTable.length);
	    var oTd = oTr.insertCell(0);
	    oTd.setAttribute('style','text-align:right;width:15%');	
	    oTd.innerHTML='幣別代號:';
	    var oTd = oTr.insertCell(1);					
		if(txtword==2){   //如果是修改		                
		   oTd.innerHTML="<input type='text' name='c00update' id='crncyno' class='txt' style='background-color:#B9B9FF;width:45%;' maxlength='4' readOnly=true  />"; 
		}else{
		   oTd.innerHTML="<input type='text' name='c00update' id='crncyno' class='txt' style='width:45%;' maxlength='4'/>"; 
		}			 
		var oTd = oTr.insertCell(2);	   
		oTd.setAttribute('style','text-align:right;width:15%');					
		oTd.innerHTML='幣別名稱:';
		var oTd = oTr.insertCell(3);               
		oTd.innerHTML="<input type='text' name='c00update' id='crncyname' class='txt' style='width:50%;' maxlength='8'    />";  				
		var oTr=ajTable.insertRow(ajTable,ajTable.length);  //以下第一列都隱藏起來當變數
		var oTd = oTr.insertCell(0);	             
		oTd.innerHTML='紀錄號碼';
		var oTd = oTr.insertCell(1);
		oTd.colspan=3;
		oTd.innerHTML="<input type='text' name='c00update' id='rcrd_no' class='txt' maxlength='14' autosize  />";                 
		oTr.setAttribute("style","display:none;");		 
		 ...
		 oTd.innerHTML='匯率:';
		 var oTd = oTr.insertCell(1);               
		 oTd.innerHTML="<input type='number' name='c0Zupdate' value=0 class='txt' id='rte3h' style='width:20%;' />";  				 
		 var oTr=ajTable.insertRow(ajTable,ajTable.length);
		 var oTd = oTr.insertCell(0);
		 oTd.setAttribute('style','text-align:right;width:15%');	
		 oTd.innerHTML='三旬日期:';
		 var oTd = oTr.insertCell(1);
		 if(txtword==2){   //如果是修改	
			oTd.innerHTML="<input type='date' name='c0Zupdate' class='txt' id='dtestd3'  style='background-color:#B9B9FF;width:35%;' readOnly=true />"; 
		 }else{
			oTd.innerHTML="<input type='date' name='c0Zupdate' class='txt' id='dtestd3'  style='width:35%;'  />"; 
		 }					
		 ...
		 oTd.innerHTML='紀錄號碼';
		 var oTd = oTr.insertCell(1);                 
		 oTd.innerHTML="<input type='text' name='c0Zupdate' id='rcrd_no' class='txt' maxlength='14' autosize  />";                 
		 oTr.setAttribute("style","display:none;");	
	}	
}
	...
	switch (txtword) {
		...
		case 2:                                                     //如果是修改，要先顯示目前該筆資料
			if (tbno==0){
			    ...
			    var editinit=document.getElementsByName('c00update');
			}else{
				 document.getElementById("rte3h").focus();				  			 				  
				 var editinit=document.getElementsByName('c0Zupdate');					    						 
			}	
...
function searchOptionsKey(tbno,slt5){	
	if (tbno==0){
        slt5.options.add(new Option('幣別代號','c00.F01'));
		slt5.options.add(new Option('幣別名稱','c00.F04'));
	}else{   
		slt5.options.add(new Option('日    期','c0Z.F02'));
	} 
}
