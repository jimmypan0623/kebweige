...
function TableToJson(args){	
	//var args=arguments; //記錄傳進了的參數
     ...
     request.open("POST", "C00/C00wrt.php", true);        //新增記錄的php檔
	...
	for(var j=1;j<c00elements.length;j++){
        if(c00elements[j].value.trim()==="" ){		
		   if(!c00elements[j].nextSibling){
		      ...
		      c00elements[j].parentNode.appendChild(errorSpan1);				 
		   }	 
		   c00elements[j].focus();
		   return false ;
        }else{		     
...
function delConfirm(event){     //確定刪除
	...
	var target=getEventTarget(event);		
		...
		request.onreadystatechange = respond;		 
		var url="C00/C00del.php?timestamp="+new Date().getTime();	     		 
		...
		function respond(){           
		  if (request.readyState == 4 && request.status == 200) {     
			  ...
			  if(!isNaN(Number(rsp))){  //如果是數字                      
					   ...
					   var valueshows=document.getElementsByName("c00value");
