...
/*用JavaScript DOM 的建立方式建立如上列顯示之html標籤，同時把該有的css屬性加入以作彈跳視窗*/
	...
	dropsheet.appendChild(dropsheet_content);  //訊息內框加入	
				...
				/*  formJason.appendChild(rspnsv); */
	             ...
	             var url="A01/BKND/A01srch.php?timestamp="+new Date().getTime(); 
                				
	             request.open("POST",url);	 
	             request.setRequestHeader("Content-Type","application/x-www-form-urlencoded");		
                 var stuffno=prsno.value; 			
				 //var queryString ="filename="+stuffno;     
				  var secondkey=document.getElementById('whono').value;
			     if(secondkey.trim().length>0){
				    var queryString ="filename="+"'"+document.getElementById('fatherkey').innerHTML+"') AND `F01` REGEXP '"+document.getElementById('whono').value+"'";
	             }else{
