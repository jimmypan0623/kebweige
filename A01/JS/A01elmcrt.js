...
/* 	var keynames=getElementsByAttribute('name','keyname');	
    ...
    var array5 = ['欄位順序','欄位名稱','讀寫來源', '是否顯示','顯示位置','寬度%','欄位內容','關聯式'];
	var array6 = ['6%','12%','6%', '6%','6%','4%','20%','20%'];
	for (var k = 0; k < array5.length; k++) {
		var th3 = document.createElement('th'); //column		   
		var text = document.createTextNode(array5[k]); //cell		
		th3.style.width=array6[k];
		th3.appendChild(text);
		thr3.appendChild(th3);		
	}			 */
/* 	var keynames=getElementsByAttribute('name','keyname');	
	keynames[1].innerHTML="功能編號:"; */
	//var fatherkeys=getElementsByAttribute('name','fatherkey');	
