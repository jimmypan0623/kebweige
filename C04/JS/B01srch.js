...
/*用JavaScript DOM 的建立方式建立如上列顯示之html標籤，同時把該有的css屬性加入以作彈跳視窗*/
	...
	dropsheet_content.style.width="80%";	
	...
	dialogButton1.setAttribute("style","position:relative;left:250px;color:green;font-size:17px;");		
	...
	dialogButton3.setAttribute("style","position:relative;left:260px;color:red;font-size:17px;");
	...
	var array = ['料品編號', '品名規格','客戶品號','有效日期','報價'];
	...
	formJason.appendChild(srchTable);
	 ...
	 var url="C04/BKND/B01srch.php?timestamp="+new Date().getTime();   	               				 
	 ...
	 if (noorname.id=='stockno'){		 
		...
		var queryString ="filename="+"b01.F01"+","+stockno+","+left(cstno,6)+","+document.getElementById('crncy').innerHTML;  
	 }else if(noorname.id=='stockname'){
		 var stockname=document.getElementById('stockname').value; 
		 var queryString ="filename="+"b01.F02"+","+stockname+","+left(cstno,6)+","+document.getElementById('crncy').innerHTML; 
	 }		 
		...
		for(var jk in arr[i]){		   
		 var oTd = oTr.insertCell(oTr.cells.length);
		  ...
		  if(jk=='stock_no'){
		        ...
		        oTd.setAttribute("style","text-align:left;width:25%;");
		  }else if(jk=='invalid_date'){	
		        oTd.setAttribute("style","text-align:center;");
		   }else if(jk=='order_price' ){	
		        oTd.setAttribute("style","text-align:center;");	
