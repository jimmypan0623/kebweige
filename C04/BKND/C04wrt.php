...
if($rows1==0 || $rows2==0){
    //if($_COOKIE["INT_013"]=="Y" && $_COOKIE["INT_099"]=="Y" ){
	...
	if($rows1==2) echo json_encode("業務人員資料錯誤");  
}else{
     ...
     if($brr[$mArlth-2]==0){        //如果旗標指示為新增		
		...
		if($rows>0){			 
        	...
        	//以下處理MySQL記錄新增  	        
	           $mscnt="INSERT INTO c03(F01,F03,F02,F07,F12,F14,F06,F13,F10)  VALUES (";  //先把準備插入記錄的SQL 語法前半段先寫在字串中	 			   
	           $mscnt.="'".$brr[0]."',";
	           $mscnt.="'".$brr[1]."',";
   	           $mscnt.="'".$brr[2]."',";	 
               $mscnt.="'".$brr[3]."',";	 	
