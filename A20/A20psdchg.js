...
function tab2View(event){	  
       ...
       var maintable=document.getElementById("member");		//所指向的表頭紀錄		 				 	 
	   ...
	   commontemp(fthkey.value,"a22.F05");
						  
}
function sourceAccount(){   //尋找被點選的紀錄
    var tabs=getElementsByAttribute('class','tab');
	if (tabs[0].checked){
	   var maintable=document.getElementById("member");
	}else{
	   maintable=document.getElementById("member2");
	}
	var respAccount; 
		...
		for(var i=0;i< maintable.rows.length; i++){
		      ...
		      if(maintable.rows[i].cells[maintable.rows[i].cells.length-1].childNodes[0].checked){
			     respAccount=maintable.rows[i].cells[0].innerHTML;
				 break;
			  }
		}
		return respAccount; 
