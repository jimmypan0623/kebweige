    ...
    if (isNaN(Number(txtword))){         //如果傳進來的參數是字串
		...
		dropsheet_content.appendChild(p_tx);				
			 ...
			 if(txtword==2){   //如果是修改
				 ...
				 var headtitle="部門編號："+aWaitUpdate[1];
		     } 
	        ...
	        if(txtword==2 || txtword==4 || txtword==5){
			    ...
			    dialogButton1.setAttribute("id","cmndbtn");
				if(txtword==2){
            ...
            oTd.appendChild(dialogButton3);		   		  //新增只有一個按鈕加入畫面  
			 ...
			 if(txtword==1 || txtword==2){  //如果是新增或修改
	            ...
	            oTd.innerHTML="<input type='text' name='belong_to' id='belong_to' class='txt' maxlength='5' autosize  />"; 
	            ...
	            oTd.innerHTML='部門性質';
	            var oTd = oTr.insertCell(1);
			    oTd.innerHTML="<div><input name='typeOfChange' type='checkbox' id='tpchg1' value='Y'><label>列入庫存&nbsp&nbsp</label>"+
                              "<input name='typeOfChange' type='checkbox' id='tpchg2' value='Y'><label>是否生產&nbsp&nbsp</label>"+
				   		      "<input name='typeOfChange' type='checkbox' id='tpchg3' value='Y'><label>計算成本  </label></div>";
	            ...
	            oTd.innerHTML='部門名稱';
	            var oTd = oTr.insertCell(1);
	            oTd.innerHTML="<input type='text' name='depart_name' id='depart_name' class='txt' maxlength='8' autosize  />"; 	 			
		        var oTr=ajTable.insertRow(ajTable,ajTable.length);
	            var oTd = oTr.insertCell(0);
	            oTd.setAttribute('style','text-align:right');
	            oTd.innerHTML='部門編號';
	            var oTd = oTr.insertCell(1);
	            oTd.innerHTML="<input type='text' name='depart_no' id='depart_no' class='txt' maxlength='6' autosize  />"; 
				 if(txtword==2){   //如果是修改	
             ...
             if (txtword==1){                                     //如果是新增
		         ...
		         var thtdy=(showTime.innerHTML.substring(0,4)+'-'+showTime.innerHTML.substring(5,7)+'-'+showTime.innerHTML.substring(8,10)); //中間一定要用減號分隔年月日
				  ...
				  document.getElementById("depart_no").value=aWaitUpdate[1];       //此時要把insue_no當成是紀錄號碼的變數
				  document.getElementById("depart_name").value= aWaitUpdate[2];  //部門名稱
				  document.getElementById("tpchg1").checked=(aWaitUpdate[3]=="Y");
				  document.getElementById("tpchg2").checked=(aWaitUpdate[4]=="Y");   //日期都設為紀錄內容
			      document.getElementById("tpchg3").checked= (aWaitUpdate[5]=="Y");  //日期都設為紀錄內容
			      document.getElementById("remark").value= aWaitUpdate[6]; 
				  document.getElementById("belong_to").value=aWaitUpdate[7]; 
...
/////  將table內容資料轉為jason
function TableToJson(departno,departname,invinc,prodinc,costinc,remark,belongto,nbrflag) {   //由此紀錄剩餘的table內容開始轉jason資料
   
	//以下為新增內容
	 		
        var rsp="";
        
	    var order_head="{"+"\""+"Departno"+"\""+":"+"\""+departno+"\""+",";	
	    order_head+="\""+"Departname"+"\""+":"+"\""+departname+"\""+",";	    	
		order_head+="\""+"Invinc"+"\""+":"+"\""+invinc+"\""+",";
		order_head+="\""+"Prodinc"+"\""+":"+"\""+prodinc+"\""+",";
		order_head+="\""+"Costinc"+"\""+":"+"\""+costinc+"\""+",";
		order_head+="\""+"Remark"+"\""+":"+"\""+remark+"\""+",";
		order_head+="\""+"Belongto"+"\""+":"+"\""+belongto+"\""+",";			
    ...
    var str_json=JSON.stringify(json);	 
     ...
     request.open("POST", "A09/A09wrt.php", true);        //新增記錄的php檔
        ...
        if (request.readyState == 4 && request.status == 200) {     
			...
			if(!isNaN(Number(rsp))){     //如果回傳為新增記錄號碼表示新增成功就做表格插入一列
               ...
               var oTd = oTr.insertCell(oTr.cells.length);
				 ...
				 oTd.setAttribute("style","text-align:left;width:102px;");
				 var oTd = oTr.insertCell(oTr.cells.length);
				 oTd.innerHTML=departname;
				 oTd.setAttribute("style","text-align:left;width:102px;"); 
				 var oTd = oTr.insertCell(oTr.cells.length);
				 oTd.innerHTML=invinc;
				 oTd.setAttribute("style","text-align:center;width:102px;"); 
				 var oTd = oTr.insertCell(oTr.cells.length);
				 oTd.innerHTML=prodinc;
				 oTd.setAttribute("style","text-align:center;width:102px;"); 
				 var oTd = oTr.insertCell(oTr.cells.length);
				 oTd.innerHTML=costinc;
				 oTd.setAttribute("style","text-align:center;width:102px;"); 
				 var oTd = oTr.insertCell(oTr.cells.length);
				 oTd.innerHTML=remark;
				 oTd.setAttribute("style","text-align:left;width:340px;");
			     var oTd = oTr.insertCell(oTr.cells.length);
				 oTd.innerHTML=belongto;
...
function sendFilePrc(nbrflg){     //新增資料上傳檔案及修改程序
	...
	var departNo=document.getElementById("depart_no");	
    var departName=document.getElementById("depart_name");		
	var checkboxes = document.getElementsByName("typeOfChange");       
                  
    var invInc=(checkboxes[0].checked?'Y':' ');
    var prodInc=(checkboxes[1].checked?'Y':' ');
	var costInc=(checkboxes[2].checked?'Y':' ');
	
	var reMark=document.getElementById("remark");	    
	var belongTo=document.getElementById("belong_to");
	var uploadFilebtn=document.getElementById("btn_uploadfile");	 
    if (nbrflg==1){     //如果是新增
	 
	    if(!/^[a-zA-Z0-9]/.test(departNo.value)){
		    if(!departNo.nextSibling){
		      ...
		      departNo.parentNode.appendChild(errorSpan1);
		    }	 
		    departNo.focus();
		    return false ;
        }else{
		    if(departNo.nextSibling){
		      departNo.parentNode.removeChild(departNo.nextSibling);
		    }
	    }
		 
    }  
		if(departName.value===""){
		    if(!departName.nextSibling){
		      var errorSpan1=document.createElement("span");
			  errorSpan1.style.color="red";
			  errorSpan1.style.fontFamily="標楷體";
		      var errorMessage1=document.createTextNode("此欄不得空白");
		      errorSpan1.appendChild(errorMessage1);
		      errorSpan1.classId="errorMsg";
		      departName.parentNode.appendChild(errorSpan1);
		    }	 
		    departName.focus();
		    return false ;
        }else{
		    if(departName.nextSibling){
		      departName.parentNode.removeChild(departName.nextSibling);
		    }
	    } 


   if(nbrflg==1){    //如果有新增 
      if(departNo.value!="" && departName.value!="" )	
        var rspns=TableToJson(departNo.value,departName.value,invInc,prodInc,costInc,reMark.value,belongTo.value,0); 
      else
		...
		for(var i=1;i< maintable.rows.length; i++){
		 ...
		 myThgrp=maintable.getElementsByTagName("th");
		 myThgrp[myThgrp.length-1].childNodes[0].textContent="選取";			
	     var rspns=TableToJson(departNo.value,departName.value,invInc,prodInc,costInc,reMark.value,belongTo.value,recordNo.value); 	   
	     blkshow("修改成功"); 		
   }
   blocksclose();			//關掉原視窗	
   return true;	 	
...
function delConfirm(event){     //確定刪除
	...
	var target=getEventTarget(event);	
		...
		request.onreadystatechange = respond;
		var url="A09/A09del.php?timestamp="+new Date().getTime();	     
