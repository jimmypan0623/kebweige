...
　  var nowday=Today.getFullYear()+ "-" + paddingLeft((Today.getMonth()+1).toString(),2) + "-" + paddingLeft((Today.getDate()).toString(),2) ;
	...
	var targetTrs=targetTbody.getElementsByTagName("tr");
	    ...
	    for (var i=0;i<targetTbody.rows.length;i++){
			if (targetTrs[i].cells[1].innerHTML==nowday &&targetTrs[i].cells[4].innerHTML==myAccount && targetTrs[i].cells[10].innerHTML=='' ){ 
			    
	           flg=Number(targetTrs[i].cells[0].innerHTML);
					 
			}
		}
	   
        if(flg>1){
		   blkshow("此頁有您未歸紀錄，請先帰社簽到再新增！");
		   return false;
