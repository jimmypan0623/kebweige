...
/*用JavaScript DOM 的建立方式建立如上列顯示之html標籤，同時把該有的css屬性加入以作彈跳視窗*/
   ...
   dialogButton1.setAttribute("value","\u{2713}");			//確認				 
	 dialogButton1.setAttribute("style","position:relative;left:190px;color:green;font-size:17px;");
	...
	dialogButton3.setAttribute("value","\u{2715}"); //關閉
	 dialogButton3.setAttribute("style","position:relative;left:200px;color:red;font-size:17px;");
	...
	formJason.appendChild(srchTable);
	 ...
	 var url="C21/BKND/B01srch.php?timestamp="+new Date().getTime();   	               				 
	 ...
	 if (noorname.id=='stockno'){		 
		...
		var queryString ="filename="+"b01.F01"+"|"+stockno+"_"+left(cstno,6);    
	 }else if(noorname.id=='stockname'){
		 var stockname=document.getElementById('stockname').value; 
		 var queryString ="filename="+"b01.F02"+"|"+stockname+"_"+left(cstno,6); 
	 }else{
		 var custompartno=document.getElementById('custompartno').value; 
		 var queryString ="filename="+"c34.F03"+"|"+custompartno+"_"+left(cstno,6); 
	 }					 
