	   ...
	   if (target.value=="\u{274E}"  && getCookie('INT_013')=='Y'){    
	      ...
	      if(document.getElementById('queryno')!=null){			  
	         ...
	         if (currentNo.trim()!="" && currentNo.trim()!=query_no){ //如果非修改且自動編號
			    const regexA = /^BH\d{2}[1-9A-C]\d{5}$/;
...
function srcArgobj(srcId){
	...
	if(srcId=='deptno1'){
       ...
       return {"headtitle":"請選取盤差部門","drpshtWidth":"28%","urlPth":"B06/BKND/A14srch.php","clickfunc":deptchoose1,"qryString":qrystring,"mendwidth":"calc( 100% - 1em )"};    
