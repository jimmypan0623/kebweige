...
foreach($cart as $key=>$val){	   
 ...
 $trnarray=fldafterwrite('B09','1',$link,true);  
...
if($rows2==0){
    if($_COOKIE["INT_099"]=="Y" ){
     ...
     if($brr[$mArlth-2]==0){        //如果旗標指示為新增		
	    ...
	    $sql="select * from b09 where F01="."'".$brr[0]."'"; 
		...
		if($rows>0){			 
        	...
        	//以下處理MySQL記錄新增  	        
	           $mscnt="INSERT INTO b09(F01,F02,F05,F08,F90,F10,F11)  VALUES (";  //先把準備插入記錄的SQL 語法前半段先寫在字串中	 			   
	           $mscnt.="'".$brr[0]."',";	      
	   ...
	   $mscnt="UPDATE b09 SET F02="."'".str_pad(trim($brr[1]),2,"0",STR_PAD_LEFT)."',";	    
	   $mscnt.="F05="."'".$brr[2]."',";	   	    	  
	   $mscnt.="F08="."'".$brr[3]."',";	 	   
