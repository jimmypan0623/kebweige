...
foreach($cart as $key=>$val){	   
 ...
 $trnarray=fldafterwrite('B10','2',$link,true);  
...
if($rows2==0){
    echo json_encode("無此品號!"); 		 
     ...
     if($brr[$mArlth-2]==0){        //如果旗標指示為新增		 
	    $sql="SELECT * FROM b1z WHERE F01='".$brr[0]."' AND F03='".$brr[1]."' "; 
		...
		if($rows>0){			 
        	...
        	//以下處理MySQL記錄新增  	        
	           $mscnt="INSERT INTO b1z(F01,F03,F04,F25,F11)  VALUES (";  //先把準備插入記錄的SQL 語法前半段先寫在字串中	 			   
	           $mscnt.="'".$brr[0]."',";
	           $mscnt.="'".$brr[1]."',";
   	           $mscnt.="'".$brr[2]."',";	 
               $mscnt.="'".$brr[3]."',";	               
	   ...
	   $mscnt="UPDATE b1z SET F04=".$brr[2].",";	    	 
	   $mscnt.="F25="."'".$brr[3]."',";	 	   
