...
require_once("../../include/BKND/auth_check.php"); //驗證
   ...
   if (substr($_POST['filename'],0,3)=="PGE"){	  
	   ...
	   $sql3="SELECT b10.*,
	        a14.F02 as F0B FROM b10 
	        LEFT OUTER JOIN `a14` ON b10.F05=a14.F01  	       
			WHERE b10.F90='".$pgeno."' ORDER BY b10.F01 DESC";	   
   }else{
		...
		$sql3="SELECT b10.*,
	        a14.F02 as F0B FROM b10  
	        LEFT OUTER JOIN `a14` ON b10.F05=a14.F01  	        
		    WHERE b10.F90='".$pgeno."' and ".$fieldNo." like '%".trim($filterKey)."%' order by ".$fieldNo.",b10.F02 DESC " ; 
   }	   
   $sql0="select * from a23 where F01='".$pgeno."'"; 
     $sql1=@mysqli_query($link,$sql0);                           
     $list4=mysqli_fetch_assoc($sql1);  //紀錄當前月份是否已結轉月庫存報表 
    $wthary=fldwdthpre('B10','1',$link); 	 	 
	$arr=array();	
    $result=@mysqli_query($link,$sql3); 
	$afld=['F00','F01','F02','F05','F0B','F08','F10','F11'];
