	   ...
	   if (target.value=="\u{274E}"  && getCookie('INT_127')=='Y'){    
	      ...
	      if(document.getElementById('queryno')!=null){			  
	         ...
	         if (currentNo.trim()!="" && currentNo.trim()!=query_no){ //如果非修改且自動編號		         
				 ...
				 discardNoRec('BI'+thtdy.substring(2,4)+parseInt(thtdy.substring(5,7)).toString(16).toUpperCase(),currentNo.trim());
	         } 
	      }
	   }
    }
...
function sendFilePrc(updflg){     //新增資料及修改程序       
    ...
    if (tbno==0){
	    var b10elements=document.getElementsByName('b10update');
        var b10athments=document.getElementsByName('b10others');			
	}else{
		 var b10elements=document.getElementsByName('b1zupdate');	
		 var b10athments=document.getElementsByName('b1zothers');			 
	}
	for(var r=0;r<b10athments.length;r++){        //關聯資料
		    nonjsn.push(b10athments[r].tagName.toUpperCase()=='SPAN'?b10athments[r].innerHTML:b10athments[r].value);		
	}
	for(var q=1;q<b10elements.length;q++){  	    //開始堆疊待異動資料陣列
		 tbjsn.push(b10elements[q].value);	   
	}
	for(var j=1;j<b10elements.length-1;j++){
		if(tbno==0 && j==2){
			  let oDate=new Date(document.getElementById('recmth').value+'-'+b10elements[j].value);
			  ...
			  let iDate=paddingLeft(b10elements[j].value.trim(),2);
			  let result = (iYear == cYear) && (iMonth == cMonth) && (iDate == cDate);
			  if(!result){
				  filtermsg(b10elements[j],"日期格式不對");
				  return false ;
			  }else{
	    	    if(b10elements[j].nextSibling){		      
			       b10elements[j].parentNode.removeChild(b10elements[j].nextSibling);
		        }		
	          }
	    }
		
        if(b10elements[j].value.trim()=="" && !(j==3 && tbno==1) && !(j==5  && tbno==0)){		//|| j==8 || j==9
		     
			    b10elements[j].placeholder="不得空白" ;
		   ...
		   if(tbno==1 && j==2){			      
		      if(b10elements[j].value == 0){
			     filtermsg(b10elements[j],"不得為 0");
				  return false ;
			  }
		     
		   } 
		   if(tbno==0 && j==4){
		      if(b10elements[4].value==b10elements[3].value){
				   filtermsg(b10elements[j],"轉出跟轉入部門不得相同!");
				  return false ;
			  }
		   }
	    }	    
...
function modifyFields(tbno,txtword,ajTable,aWaitUpdate){   //新增修改時出現之欄位
    if (tbno==0){   //如果異動表頭資料			     				
	    ...
	    oTd.innerHTML="<input type='text' name='b10update' id='shipdirect' class='txt' style='width:55%;' maxlength='40'    />";  		
	    ...
	    oTd.innerHTML='盤差部門:';
	    var oTd = oTr.insertCell(1);               	              
	    oTd.innerHTML="<input type='text' name='b10update' id='deptno1' class='txt' style='width:15%;' maxlength='6'    />";  				  
	    oTd.innerHTML+="<span name='b10others' id='deptname1'></span>&nbsp&nbsp";  
	    ...
	    oTd.innerHTML='盤差日:';
	    var oTd = oTr.insertCell(1);            		
	    oTd.innerHTML="<input type='text' name='b10update' id='shipdate' class='txt' style='width:8%;' maxlength='2'   />";  	 	 			
	    var oTr=ajTable.insertRow(ajTable,ajTable.length);
	    var oTd = oTr.insertCell(0);
	    oTd.setAttribute('style','text-align:right;width:15%');	
	    oTd.innerHTML='盤差單號:';
	    ...
	    oTd.colspan=3;				  
	    if(txtword==2){   //如果是修改		                    
	 	   oTd.innerHTML="<input type='text' name='b10update' id='queryno' class='txt' style='background-color:#B9B9FF;width:25%;' maxlength='10' readOnly=true  />";		    				 	         
		}else{
		   oTd.innerHTML="<input type='text' name='b10update' id='queryno' class='txt' style='width:25%;' maxlength='10'/>"; 		     	 
	    }			             
	    var oTr=ajTable.insertRow(ajTable,ajTable.length);  //以下第一列都隱藏起來當變數
	    var oTd = oTr.insertCell(0);	             
	    oTd.innerHTML='紀錄號碼';
	    var oTd = oTr.insertCell(1);	  
	    oTd.innerHTML="<input type='text' name='b10update' id='rcrd_no' class='txt' maxlength='14' autosize  />";                 
	    ...
	    oTd.innerHTML='備註:';
	    var oTd = oTr.insertCell(1);		
		oTd.innerHTML="<input type='text' name='b1zupdate' id='extradsp' class='txt' style='width:50%;' maxlength='20'/>"; 				 		    
 	    var oTr=ajTable.insertRow(ajTable,ajTable.length);
	    var oTd = oTr.insertCell(0);	   
	    oTd.setAttribute('style','text-align:right;width:15%');					
   	    oTd.innerHTML='數量:';
	    var oTd = oTr.insertCell(1);      
	    oTd.innerHTML="<input type='number' name='b1zupdate' id='queryqty' value=1 class='txt' style='width:20%;text-align:right;' />";	    
	    var oTr=ajTable.insertRow(ajTable,ajTable.length);
	    var oTd = oTr.insertCell(0);	   
	    oTd.setAttribute('style','text-align:right;width:15%');					
	    oTd.innerHTML='品名規格:';
	    var oTd = oTr.insertCell(1);      
	    if(txtword==2){   //如果是修改
		   oTd.innerHTML="<input type='text' name='b10others' id='stockname' class='txt' style='background-color:#B9B9FF;width:70%;' maxlength='40' readOnly=true />";  				 
	    }else{
		   oTd.innerHTML="<input type='text' name='b1zothers' id='stockname' class='txt' style='width:70%;' maxlength='40'    />";  				 
	    ...
	    var oTd = oTr.insertCell(1);     
	    if(txtword==2){   //如果是修改	
		    oTd.innerHTML="<input type='text' name='b1zupdate' id='stockno' class='txt' style='background-color:#B9B9FF;width:60%;' maxlength='43' readOnly=true  />";  				              
	    }else{
		    oTd.innerHTML="<input type='text' name='b1zupdate' id='stockno' class='txt' style='width:60%;' maxlength='43'    />";
	    ...
	    oTd.innerHTML='紀錄號碼';
	    var oTd = oTr.insertCell(1);                 
	    oTd.innerHTML="<input type='text' name='b1zupdate' id='rcrd_no' class='txt' maxlength='14' autosize  />";                 
	    oTr.setAttribute("style","display:none;");	
	}				  			             	
}
...
function searchOptionsKey(tbno,slt5){	
    if (tbno==0){		 
		 slt5.options.add(new Option('報廢單號','b10.F01'));		  		 
		 slt5.options.add(new Option('報廢日期','b10.F02'));
		 slt5.options.add(new Option('轉出部門編號','b10.F05')); 
		 slt5.options.add(new Option('轉差部門名稱','a14.F02')); 		
		 slt5.options.add(new Option('已確認?(Y/N)','b10.F10')); 			
	}else{
		 slt5.options.add(new Option('料品編號','b1z.F03'));
...
function  addNewRecordHint(tbno){
    if (tbno==0){  //表頭資料
	   ...
	   return "請輸入報廢單:"+sourceAccount(1,0)+"內容資料："; 
    }		
}
...
function editRecordHint(tbno){
    if (tbno==0){  
		...
		return "修改報廢單:"+sourceAccount(1,0)+"內容資料："; 
	}	 
}
...
function srcArgobj(srcId){
	...
	if(srcId=='deptno1'){
       ...
       return {"headtitle":"請選取報廢部門","drpshtWidth":"28%","thCntnt":['部門編號', '部門名稱'],"thWidth":['50%','50%'],"urlPth":"B06/BKND/A14srch.php","clickfunc":deptchoose1,"qryString":qrystring,"mendwidth":"calc( 100% - 1em )"};    	
	}else{
...
function deptchoose1(event)  //報廢部門編號選擇
{
	if (typeof event=="undefined"){
		event=window.event;
	}
