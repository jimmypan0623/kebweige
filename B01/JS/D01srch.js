...
/*用JavaScript DOM 的建立方式建立如上列顯示之html標籤，同時把該有的css屬性加入以作彈跳視窗*/
	...
	dropsheet_content.setAttribute("id","modal-content4");
	...
	dropsheet.appendChild(dropsheet_content);  //訊息內框加入	
				...
				/*  formJason.appendChild(rspnsv); */
	             ...
	             var url="B01/BKND/D01srch.php?timestamp="+new Date().getTime();   	               				 
	             request.open("POST",url);	 
	             request.setRequestHeader("Content-Type","application/x-www-form-urlencoded");		
                  		
                 if (noorname.id=='vendorno'){		 
				    var vendno=document.getElementById('vendorno').value; 
				    var queryString ="filename="+"d01.F01"+"|"+vendno;    
					
                 }else{
					 var vendname=document.getElementById('vendorname').value; 
					 var queryString ="filename="+"d01.F04"+"|"+vendname;    
                 }					 
...
function vndchse(event)  //選擇廠商
{
	...
	var shipplace=document.getElementById('dlvrplace');
	 ...
	 var maintable=document.getElementById("vendTbody");  
...
function vendblkclose(event)  //關閉註冊彈出視窗
{
	if (typeof event=="undefined"){
		event=window.event;
	}
...
function srchVendNo(str1) {       //開窗顯示人員選項
    var cnt=0;
	var arr = str1; 
   var oTable = document.getElementById("vendTbody");		
