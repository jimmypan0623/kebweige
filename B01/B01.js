...
function getProfile(str1,pagecount) {    
    ...
    var tabs=getElementsByAttribute("class","tab");
        ...
        var optdigts= Math.ceil(Math.log(pagecount)/Math.log(10));
        var slt2=document.getElementById('recmth');
	   ...
	   // if(Cookies.get('delauth')!='Y'){   //沒有刪除權限一次只能選一筆	
		...
		//}
		   ...
		   for(var i=0;i<7;i++){
			...
			for(var i=10;i<22;i++){
			    b01a_value_names[i].innerHTML=targetTrChildren[i].innerHTML;
		    }
...
function rowchoose(event){   //點選列ROW就可以選擇該筆資料
	...
	if(targetRow.lastChild.firstChild.checked){
		...
		for(var i=10;i<22;i++){
			b01a_value_names[i].innerHTML=targetRow.childNodes[i].innerHTML;
		}
