...
/*用JavaScript DOM 的建立方式建立如上列顯示之html標籤，同時把該有的css屬性加入以作彈跳視窗*/
	...
	dropsheet.appendChild(dropsheet_content);  //訊息內框加入	
		        ...
		        dialogButton1.setAttribute("value","確認");			
				 dialogButton1.setAttribute("style","position:relative;left:110px;");
				if(prsno.id=='whono'){			
				 ...
				 dialogButton3.setAttribute("style","position:relative;left:120px;");
...
function srchblkclose(event)  //關閉註冊彈出視窗
{
	...
	if (dropsheet!=null){		
	   ...
	   cko[1](bibau*(-1));    //將閉包變數歸零	
	    var bibau=cko[5](0);   //找出閉包變數現值
	   cko[5](bibau*(-1));    //將閉包變數歸零	 
	return true;
}	
		 ...
		 var oTd = oTr.insertCell(oTr.cells.length);		//再新增一欄 	
	      oTd.setAttribute("style","width:40px;display:none");   //勾選不顯示
		   ...
		   myCheck.type="checkbox";
		    ...
		    myCheck.setAttribute("name","recordchosen2");   //讓使用者勾選的checkbox表	
