...
include("../../include/BKND/mysqli_server.php");                              //引用檔   
	    ...
	    $sql3="SELECT b0b.*,d04.F01 AS F0A,d04.F09 AS F0I,d04.F24 FROM b0b,d04 WHERE b0b.F03='".$brr[1]."' AND b0b.F07='".$brr[2]."' AND d04.F09-d04.F24>=0 AND d04.F01='".$brr[2]."' AND d04.F02='".$brr[1]."' "; 		
...
if($rows2==0){
    echo json_encode("無此退貨部門"); 		 
}else if($rows1==0){
    echo json_encode("此出貨紀錄無可退或可折讓數量"); 
}else if($list1['F0I']-$list1['F24']-$brr[3]<0){
	echo json_encode("此筆退出數量超過已進數量:".strval(($list1['F0I']-$list1['F24']-$brr[3])*(-1))); 
}else{	
	...
	$sql15="SELECT * FROM b03 WHERE F01='".$brr[0]."' "; 
	$sql16=@mysqli_query($link,$sql15);
	$list5=mysqli_fetch_assoc($sql16);  //紀錄表頭折讓貨退貨	 
	
     if($brr[$mArlth-2]==0){        //如果旗標指示為新增		    
	    $sql="SELECT * FROM b0c WHERE F01='".$brr[0]."' AND F03='".$brr[1]."' AND F07='".$brr[2]."' "; 
		...
		if($rows>0){			 
        	...
        	//以下處理MySQL記錄新增  	        
	            $mscnt="INSERT INTO b0c(F01,F03,F07,F04,F15,F05,F08,F09,F12,F13)  VALUES (";  //先把準備插入記錄的SQL 語法前半段先寫在字串中	 			   
	            $mscnt.="'".$brr[0]."',";
	            $mscnt.="'".$brr[1]."',";
   	            $mscnt.="'".$brr[2]."',";	 
                $mscnt.="'".$brr[3]."',";	
			    ...
			    if(intval($list5['F24'])<3){   //非折讓就要寫入原訂單開單未過帳量
                  $armstd04="UPDATE d04 SET F24=F24+".$brr[3]." WHERE F02='".$brr[1]."' AND F01='".$brr[2]."' ";  	                                               
                  mysqli_query($link ,$armstd04) or die(mysqli_error($link));    //寫入MySQL 	
	            }
			   
		} //新增判斷或執行結束   	     
    }else{	   //修改	    
	    $mscnt="UPDATE b0c SET F04=F04+".$brr[3].",";	    
	    $mscnt.="F15="."'".$brr[4]."',";	   
	    $mscnt.="F05="."'".$brr[5]."',";	 
	    $mscnt.="F08="."'".$brr[6]."',";	 
	    $mscnt.="F09="."'".$brr[7]."',";	
