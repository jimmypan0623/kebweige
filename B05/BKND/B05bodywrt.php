...
// 對齊修法：前端已改為單次 stringify(真實物件)，這裡直接一次解碼
...
try {
    ...
    if ($stmt5->get_result()->num_rows === 0) {
        throw new Exception("無此收貨部門");
    }

    // B. 檢查出貨紀錄與可退/可折讓數量 (串接 b0d 與 c04，FOR UPDATE 鎖定行防止併發問題)
    $sql_plan = "SELECT b0d.*, c04.F01 AS F0A, c04.F09 AS F0I, c04.F24 
                 FROM b0d 
                 JOIN c04 ON c04.F01 = b0d.F07 AND c04.F02 = b0d.F03
                 WHERE b0d.F03 = ? AND b0d.F07 = ? AND (c04.F09 - c04.F24) >= 0 AND c04.F01 = ? AND c04.F02 = ?
                 FOR UPDATE";
    ...
    if ($qty_diff < 0) {
        throw new Exception("此筆退回數量超過已出數量:" . strval(abs($qty_diff)));
    }
    ...
    $stmt_b05 = $link->prepare("SELECT F24 FROM b05 WHERE F01 = ?");
    $stmt_b05->bind_param("s", $brr[0]);
    $stmt_b05->execute();
    $list5 = $stmt_b05->get_result()->fetch_assoc();
    ...
    if ($mode_id == 0) {
        // 新增模式：檢查重複品號與單號
        $stmt_check = $link->prepare("SELECT F00 FROM b0e WHERE F01 = ? AND F03 = ? AND F07 = ?");
        ...
        // 寫入銷貨退回明細表 b0e
        $sql_ins = "INSERT INTO b0e (F01, F03, F07, F04, F15, F05, F08, F09, F12, F13) VALUES (?,?,?,?,?,?,?,?,?,?)";
        ...
        // 修改模式：更新 b0e 明細
        if (!empty($brr[8])) {
            $sql_upd = "UPDATE b0e SET F04 = F04 + ?, F15 = ?, F05 = ?, F08 = ?, F09 = ?, F12 = ?, F13 = ? WHERE F00 = ?";
            $stmt_upd = $link->prepare($sql_upd);
            $stmt_upd->bind_param("dssssssi", $brr[3], $brr[4], $brr[5], $brr[6], $brr[7], $brr[8], $update_tag, $mode_id);
        } else {
            $sql_upd = "UPDATE b0e SET F04 = F04 + ?, F15 = ?, F05 = ?, F08 = ?, F09 = ?, F13 = ? WHERE F00 = ?";
            $stmt_upd = $link->prepare($sql_upd);
    ...
    // 非折讓單（F24 < 3）時，回寫出貨單明細 c04 累計退貨量
    if (isset($list5['F24']) && intval($list5['F24']) < 3) {
        $stmt_upd_c04 = $link->prepare("UPDATE c04 SET F24 = F24 + ? WHERE F02 = ? AND F01 = ?");
