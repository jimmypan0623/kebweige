...
foreach($cart as $key=>$val){	   
 ...
 $trnarray=fldafterwrite('B05','1',$link,true);  
     $sql5="SELECT * FROM a01 WHERE F01='".$brr[3]."'"; 
		 ...
		 if($brr[$mArlth-2]==0){
	        $sql3="SELECT * FROM b04 WHERE F01='".$brr[5]."' AND F20='".$brr[6]."' AND F06='".$brr[1]."' "; 
...
if($rows1==0 || $rows2==0){
    ...
    if($brr[$mArlth-2]==0){        //如果旗標指示為新增		
		...
		if($rows>0){			 
        	...
        	//以下處理MySQL記錄新增  	        
	           $mscnt="INSERT INTO b05(F01,F06,F02,F09,F08,F21,F20,F22,F23,F14,F16,F24,F25,F90,F10,F13) VALUES (";  //先把準備插入記錄的SQL 語法前半段先寫在字串中	 			   
	           $mscnt.="'".$brr[0]."',";
	           $mscnt.="'".$brr[1]."',";
