...
foreach($cart as $key=>$val){	   
 ...
 if($list2['F07']!='Y'){
	 ...
	 $sqlA="SELECT a03.F01,a03.F02,MID(a04.F08,1,3) AS SRCSDB FROM a03,a04 WHERE a03.F01=a04.F01 AND MID(a03.F03,2,2)='MR' AND a04.F02='100' ORDER BY a03.F01 ";
	 $sqlB=@mysqli_query($link,$sqlA);
	 $nairr=array(); 
	while ($list5=mysqli_fetch_assoc($sqlB)){
		  $sqlC="SELECT `F01`,`F10` FROM `".$list5['SRCSDB']."` WHERE `F10`<>'Y' AND F90='".$brr[0]."'";
		  $sqlD=@mysqli_query($link,$sqlC);
		  $list9=mysqli_fetch_assoc($sqlD); 
		  $rows2=@mysqli_num_rows($sqlD);  
		  if($rows2>0){
			$nairr=array($list5['F01'],$list5['F02']);
			break;
	      }				 			  		 		
	}
	if(count($nairr)>0){
	    echo json_encode($nairr[0]." ".$nairr[1]."，尚有單號：<mark>".$list9['F01']."</mark>未確認!"); 
		 ...
		 if($rows1>0){
			...
			mysqli_query($link ,$sql) or die(mysqli_error($link));  	  
			 ...
			 $sql3="SELECT F01,F02,F15 FROM b25  WHERE F90='".$brr[0]."' AND F15<>0 ORDER BY F01,F02"; 
