		   ...
		   if(tbno==1 && (j==2 || j==3)){	
		      if(d04elements[j].value == 0){
			    ...
			    if(j==2 && updflg!=1){
			        ...
			        if(d04elements[2].value*1 < sourceAccount(8,1)*1+sourceAccount(9,1)*1+sourceAccount(11,1)*1){
...
function srcArgobj(srcId){
	if(srcId=='vendorno' || srcId=='vendorname'){
      ...
      return {"headtitle":"請選取採購人員帳號姓名","drpshtWidth":"28%","thCntnt":['人員編號', '人員姓名'],"thWidth":['50%','50%'],"urlPth":"C01/BKND/A01srch.php","clickfunc":chseprg1,"qryString":qrystring,"mendwidth":"calc( 100% - 1em )"};
		...
		return {"headtitle":tttlt,"drpshtWidth":"80%","thCntnt":['料品編號', '品名規格','廠商品號','有效日期','報價'],"thWidth":['30%','25%','25%','10%','10%'],"urlPth":"D04/BKND/B01srch.php","clickfunc":stckchg,"qryString":qrystring,"mendwidth":"calc( 100%  )"};
	}
}
