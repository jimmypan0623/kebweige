...
class MYPDF extends TCPDF {
    // 存放外部注入的資料 (如 ourCompany, customNo 等)
    public $customData = [];
    ...
    public function Header() {
        ...
        <table border="0" width="100%">
            <tr>
                <td style="width:70%; text-align: right; font-size: 16pt; font-weight: bold;">採購訂單 PURCHASE ORDER</td>
        ...
        if ($this->getPage() == 1) {
            ...
            <table cellpadding="1" style="font-size: 10pt;">
                <tr>
                    <td style="width:50%;">廠商編號: '.($this->customData['customNo'] ?? '').'</td>
                    <td style="width:50%; text-align:right;">採購單號: '.($this->customData['queryNo'] ?? '').'</td>
                </tr>
                <tr>
                    <td style="width:70%;">採購人員: '.($this->customData['salesMan'] ?? '').' &nbsp; 交易幣別: '.($this->customData['curNcy'] ?? '')."&nbsp;".($this->customData['curName'] ?? '').'</td>
                    <td style="width:30%; text-align:right;">下單日期: '.date('Y/m/d').'</td>
                </tr>
                <tr>
                    <td style="width:50%;">交貨地點: '.($this->customData['shipAddress'] ?? '').'</td>
                    <td style="width:50%; text-align:right;">運送方式: '.($this->customData['shipDirect'] ?? '').'</td> 
                </tr>
            </table>';
        }
...
$pdf->customData = $_GET; // 將所有參數注入物件供內部調用
