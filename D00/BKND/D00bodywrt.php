...
require_once("../../include/BKND/mysqli_server.php");                      //引用檔
 ...
 $trnarray=fldafterwrite('D00','2',$link,true);
	 ...
	 $lastdate=date('Y'.'-'.'m'.'-'.'d');
     $mArlth=count($brr);  
	 $sql3="SELECT F03 FROM d00 WHERE F01='".$brr[0]."' AND LEFT(F03,7)<='".$brr[1]."'";  //查看d00有無未更新的匯率
	 $sql4=mysqli_query($link,$sql3);
     $rows1=@mysqli_num_rows($sql4);
    if($brr[$mArlth-2]==0){        //如果旗標指示為新增						   
	    $sql="select * from d0Z where F01="."'".$brr[0]."' and F02='".$brr[1]."'"; 
		...
		if($rows>0){			 
		   ...
		   $mscnt="INSERT INTO d0Z(F01,F02,F03,F04)  VALUES (";  //先把準備插入記錄的SQL 語法前半段先寫在字串中	 			   
		   $mscnt.="'".$brr[0]."',";
		   $mscnt.="'".$brr[1]."',";
		   $mscnt.="'".$brr[2]."',";	    
		...
		$mscnt="UPDATE d0Z SET F02="."'".$brr[1]."',";	    
	    $mscnt.="F03="."'".$brr[2]."',";	   	  
