   ...
   $sql0="select * from a23 where F01='".$pgeno."'"; 
	...
	while ($list3=mysqli_fetch_assoc($sql4)){		 
		$atr = array('rc_no'.$wthary[0]=>$list3['F00'],  		            	             
                     ...
                     'receive_dptno'.$wthary[5]=>$list3['F07'],	
                     'receive_dptname'.$wthary[6]=>$list3['F0C'],     						                   
					 'remark_direct'.$wthary[7]=>$list3['F08'],  	
                     'shure'.$wthary[8]=>$list3['F10'],     					 
					 'lastupdate'.$wthary[9]=>$list3['F11']                      				 
					 );                      			
		array_push($arr,$atr);
	}
	mysqli_close($link);
