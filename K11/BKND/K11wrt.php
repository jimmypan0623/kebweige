...
if($rows1==0 || $rows2==0){
	...
	if($rows1==0) echo json_encode("無此廠商未沖銷紀錄"); 	   
    ...
    if($brr[$mArlth-2]==0){        //如果旗標指示為新增		
	    ...
	    $sql="SELECT * FROM k08 WHERE F01='".$brr[0]."'"; 
		...
		if($rows>0){			 
        	...
        	//以下處理MySQL記錄新增  	        
	           $mscnt="INSERT INTO k08(F01,F06,F02,F09,F03,F04,F05,F07,F08,F90,F10,F13,F22) VALUES (";  //先把準備插入記錄的SQL 語法前半段先寫在字串中	 			   
	           $mscnt.="'".$brr[0]."',";
	           $mscnt.="'".$brr[1]."',";
	   ...
	   $mscnt="UPDATE k08 SET F02='".str_pad(trim($brr[2]),2,"0",STR_PAD_LEFT)."',";	    
	   $mscnt.="F09='".$brr[3]."',";	   	    	  	
	   $mscnt.="F03='".$brr[4]."',";	 
	   $mscnt.="F04='".$brr[5]."',";	 
	   $mscnt.="F05='".$brr[6]."',";	 
