...
function ratechange(event){     //匯率更改異動
	...
	var url="B04/C00srch.php?timestamp="+new Date().getTime();        		
...
function rateSrch(event){   //出貨日期異動順便更動匯率
    ...
    if(Cookies.get('INT_011')!=crtNow){	
	   var sendSrcRec="filename="+crtNow+"|"+ckc.value+"|"+target.value;	
		...
		var url="B04/C0ZRateChange.php?timestamp="+new Date().getTime();
...
function c01CustomName(event){	
   if (typeof event=="undefined")
	...
	request.onreadystatechange = respond;	   
	var url="B04/C01CustomName.php?timestamp="+new Date().getTime();		
	request.open("POST",url);	 
	request.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
	request.send(sendSrcRec);		
	function respond(){           
...
function modifyFields(tbno,txtword,ajTable,aWaitUpdate){   //新增修改時出現之欄位
    if (tbno==0){   //如果異動表頭資料			     				
	    ...
	    var oTd = oTr.insertCell(1);     
	    oTd.colspan=3;
	    oTd.innerHTML="<input type='text' name='b04update' id='dlvrplace' class='txt' style='width:80%;' maxlength='40'    />";
	    var oTr=ajTable.insertRow(ajTable,ajTable.length);
	   ...
	   oTr.setAttribute("style","display:none;");   //整列隱藏
	    ...
	    oTd.innerHTML='出貨部門:';
	    var oTd = oTr.insertCell(1);               	              
	    oTd.innerHTML="<input type='text' name='b0dupdate' id='deptno' class='txt' style='width:15%;' maxlength='5'    />";  				  
